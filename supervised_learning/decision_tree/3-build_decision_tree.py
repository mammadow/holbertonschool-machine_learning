...
class Node:
    ...
    def left_child_add_prefix(self, text):
        """Adds the left-child prefix (vertical bar continuation) to each line."""
        lines = text.split("\n")
        new_text = "    +--" + lines[0] + "\n"
        for x in lines[1:]:
            if x:
    ...
    def right_child_add_prefix(self, text):
        """Adds the right-child prefix (space continuation) to each line."""
        lines = text.split("\n")
        new_text = "    +--" + lines[0] + "\n"
        for x in lines[1:]:
            if x:
    ...
    def get_leaves_below(self):
        """Returns the list of all leaves in the subtree rooted at this node."""
